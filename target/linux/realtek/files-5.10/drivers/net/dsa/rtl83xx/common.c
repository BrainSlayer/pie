...
static int rtl83xx_port_lower_walk(struct net_device *lower, struct netdev_nested_priv *_data)
{
	struct rtl83xx_walk_data *data = _data->data;
...
int rtl83xx_port_dev_lower_find(struct net_device *dev, struct rtl838x_switch_priv *priv)
{
	...
	struct netdev_nested_priv p;
	data.priv = priv;
	data.port = 0;
	p.data = &data;

	netdev_walk_all_lower_dev(dev, rtl83xx_port_lower_walk, &p);
...
static int __init rtl83xx_sw_probe(struct platform_device *pdev)
{
	...
	if (soc_info.family != RTL9300_FAMILY_ID && soc_info.family != RTL9310_FAMILY_ID) { // TODO: Port this functionality
